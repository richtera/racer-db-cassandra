...
function Cassandra (options) {
  ...
  if (this.options.solr) {
    ...
    this.solrs = [];
    var self = this;
    (this.options.solr.hosts || [this.options.solr.host]).forEach(function (host) {
      var options = {};
      Object.keys(self.options.solr).forEach(function (e) {
        if (e === 'hosts' || e === 'host')
          return;
        options[e] = self.options.solr[e];
      });
      options.host = host;
      var s = solr.createClient(options);
      self.solrs.push(s);
      s.displayName = host;
      if (self.options.solr.auth) {
        var u = self.options.solr.auth.split(':');
        s.basicAuth(u[0], u[1]);
      }
    });
  }
      ...
      if (schema) {
        ...
        if (options.enableDse) {
          ...
          function checkZookeeperConfig() {
            if (!self.aliveCores) {
              console.log('Checking STATUS of SOLR cores.');
              return self.solr.sendToSolr({
...
//                  self.solrLogger.log('Updating ' + item.path);
        ...
        function waitForCore(core, callback) {
          var timeout = setTimeout(function () {
            if (self.solrLogger)
              self.solrLogger.log('Timeout wairing for new core ' + core + ' on all SOLR nodes');
            callback(new Error('Timeout waiting for new core ' + core));
            timeout = null;
          }, 120000);
          var index = 0;
          checkOneCore();
          function checkOneCore() {
            if (index < self.solrs.length) {
              console.log(self.solrs[index]);
              if (self.solrLogger)
                self.solrLogger.log('Checking for ' + core + ' on ' + self.solrs[index].displayName);
              self.solrs[index].sendToSolr({
                path: '/admin/cores?action=STATUS&wt=json'
              }, function (err, result) {
                result = JSON.parse(result);
                var exp = new RegExp('^' + self.keyspace + '\\..*');
                ...
                Object.keys(result.status).filter(function (core) {
                  ...
                  aliveCores[core] = true;
                });
                if (aliveCores[core]) {
                  if (self.solrLogger)
                    self.solrLogger.log('Found ' + core + ' on ' + self.solrs[index].displayName);
                  index++;
                  checkOneCore();
                }
              });
              ...
              if (timeout != null) {
                ...
                if (self.solrLogger)
                  self.solrLogger.log('Found ' + core + ' on all SOLR nodes before timeout');
                callback(null);
              }
            }
          }
        ...
        function solrCreate() {
          if (self.solrLogger)
            self.solrLogger.log('Sending ' + (self.aliveCores[core] ? 'RELOAD' : 'CREATE') + ' ' + core);
          var shardArgs = [];
  ...
  if (!model) {
    ...
    function getTheModel() {
      ...
      function done(err, fields) {
        ...
        if (keys.length === 0)
          keys.push(fields.id = {primary_key:1,name:'id',type:'uuid'});
        var partitionKeys = 0;
        keys.forEach(function (k) {
          if (k.primary_key < 10)
            partitionKeys++;
        });
        if (keys.length == partitionKeys)
          partitionKeys--;
        if (partitionKeys < 1)
          partitionKeys = 1;
        ...
        function update(err) {
          ...
          if (options.remove) {
            model.destroyById(id, function (err) {
              self.indexDoc(collection, null, id, 'findAndModifyRemove', model, callback);
            });
          } else {
            doc.id = id;
            model.update(doc, conds, function (err, info) {
              ...
              self.indexDoc(collection, o, id, 'findAndModify', model, function (err2) {
                if (err2) console.log(err2);
                doc = self.unflatten(model, doc);
                doc._id = id;
                callback(err, doc);
              });
            }, options);
...
Cassandra.prototype.find = function find (collection, conds, opts, cb) {
  ...
  var self = this;
  var multicore = {};
  multicore[collection] = true;;
  var q = conds.$solr || conds.solr_query;
  if (q && q.multicore) {
    q.multicore.forEach(function (x) {
      multicore[x] = true;
    });
  }
  return this.getModel(collection, gotModel);
  function gotModel(err, model) {
    ...
    if (conds.$solr || conds.solr_query) {
      delete multicore[model.schema.info.tableName];
      ...
      if (solrQuery && (solrQuery.reindex || (self.options.solr.reindex && !self.options.solr.reindex[collection]))) {
        ...
        solrFind();
      function solrFind() {
        var mk = Object.keys(multicore);
        if (mk.length > 0) {
          return self.getModel(collection = mk[0], gotModel);
        }
        var q = solrQuery;
        solrProperty = q;
        var hl = false;
