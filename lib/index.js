      ...
      if (schema) {
        ...
        function solrCreate() {
          ...
          self.solr.sendToSolr({
            path: '/admin/' + ((options.enableDse || options.shardCount === 1) ? 'cores' : 'collections') + '?action=CREATE&name=' + core + shardArgs
          }, function (err, result) {
            if (!err) {
              if (self.solrLogger)
            ...
            self.solr.sendToSolr({
              path: '/admin/' + ((options.enableDse || options.shardCount === 1) ? 'cores' : 'collections') + '?action=RELOAD&name=' + core
            }, function (err, result) {
              if (err) {
                if (self.solrLogger) {
                  if (result)
