                ...
                if (value) {
                  ...
                  if (value === schema) {
                    self.solr.sendToSolr({
                      path: '/admin/cores?action=STATUS&core=' + core + '&wt=json'
                    }, function (err, result) {
                      result = JSON.parse(result);
                      if (err || Object.keys(result.status[core]).length == 0) {
                        if (self.solrLogger)
...
//                  self.solrLogger.log('Updating ' + item.path);
        ...
        function solrCreate() {
          ...
          self.solr.sendToSolr({
            path: '/admin/' + ((options.enableDse || options.shardCount === 1) ? 'cores' : 'collections') + '?action=RELOAD&wt=json&name=' + core + shardArgs
          }, function (err, result) {
            ...
            self.solr.sendToSolr({
              path: '/admin/' + ((options.enableDse || options.shardCount === 1) ? 'cores' : 'collections') + '?wt=json&action=CREATE&name=' + core + shardArgs
            }, function (err, result) {
              if (err) {
                if (self.solrLogger) {
                  if (result)
