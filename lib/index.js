      ...
      if (schema) {
        ...
        function solrCreate() {
          ...
          self.solr.sendToSolr({
            path: '/admin/' + (options.enableDse ? 'cores' : 'collections') + '?action=RELOAD&name=' + core + shardArgs
          }, function (err, result) {
            if (!err) {
              if (self.solrLogger)
            ...
            self.solr.sendToSolr({
              path: '/admin/' + (options.enableDse  ? 'cores' : 'collections') + '?action=CREATE&name=' + core
            }, function (err, result) {
              if (err) {
                if (self.solrLogger) {
                  if (result)
