    ...
    cassandra.schemaSync = function (model, callback) {
      ...
      if (schema) {
        ...
        self.solr.sendToSolr({ path: '/resource/' + core + '/schema.xml', method: 'GET'}, function (err, result) {
          ...
          if (a === b && self.solrInitialized[model.schema.info.tableName]) {
          ...
          async.forEach(items, function (options, next) {
            ...
            self.solr.sendToSolr({
              ...
              if (!err) {
                self.solrInitialized[model.schema.info.tableName] = true;
                return callback(err);
              }
              if (self.solrLogger) {
                if (result)
        ...
        function update(err) {
          ...
          if (options.remove) {
            ...
            model.update(doc, conds, function (err, info) {
              ...
              self.indexDoc(collection, o, id, 'findAndModify');
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (toInsert._id) {
      ...
      return model.update(json, {where:{id:toInsert._id}}, function (err, info) {
        self.indexDoc(collection, json, id, 'findAndModify');
