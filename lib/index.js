    ...
    cassandra.schemaSync = function (model, callback) {
      ...
      if (schema) {
        ...
        async.forEach(items, function (options, next) {
          self.solr.sendToSolr(options, function (err) {
            if (err) {
              if (self.solrLogger)
                self.solrLogger.log('SENDING failed ' + options.info + ' => ' + err.message);
            }
            next();
          });
        }, function (err) {
          self.solr.sendToSolr({
            ...
            if (self.solrLogger)
              self.solrLogger.log('CREATE failed, trying reload ' + core + ' => ' + err.message);
            self.solr.sendToSolr({
              path: '/admin/cores?action=RELOAD&name=' + core
            }, function (err) {
              if (err) {
                if (self.solrLogger)
                  self.solrLogger.log('RELOAD failed ' + core + ' => ' + err.message);
              }
              callback(err);
            });
          });
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (conds.$solr || (self.options.enableDse && conds.solr_query)) {
      ...
      function solrFind() {
        ...
        if (self.options.enableDse) {
          q.core(model.connection.keyspace.name + '.' + model.schema.info.tableName);
        }
        if (self.solrLogger) {
