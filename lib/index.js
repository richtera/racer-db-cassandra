...
Cassandra.prototype.indexDoc = function indexDoc(collection, doc, id, where, model) {
//  if (this.options.enableDse)
//    return;
    ...
    function sendDocument() {
      if (doc) {
        ...
        if (!o._extract) {
          ...
          options.update = 'update/extract?commit=true&fmap.content=' + destField + '&' + params;
        ...
        self.solr.delete('id', id, function (err, obj) {
          if (err) console.log(err);
          scheduleSolrCommit();
        });
      }
        ...
        model.find({where:{}}, function findCb (err, rows) {
          ...
          function addDoc() {
            ...
            if (!row) {
              ...
              var o = self.formatIndexDoc(collection, item, item.id, model);
              if (self.solrLogger) {
                self.solrLogger.log('ADD', o);
              }
