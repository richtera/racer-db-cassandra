...
Cassandra.prototype.getModel = function getModel (collection) {
...
Cassandra.prototype.cleanupId = function (doc, removeId) {
  var out = {};
  for (var i in doc) {
    if (i === '_id') {
      if (!removeId)
...
Cassandra.prototype.findAndModify = function findAndModify (collection, conds, sort, doc, options, callback) {
  ...
  conds = this.cleanupId(conds);
  doc = this.cleanupId(doc, true);
...
Cassandra.prototype.find = function find (collection, conds, opts, cb) {
  var model = this.getModel(collection);
  var filter = {
    where: this.cleanupId(conds),
    order: this.convertSort(opts.sort),
    limit: opts.limit,
    offset: opts.offset
  };
  console.log('finding');
...
Cassandra.prototype.count = function count (collection, conds, opts, cb) {
  ...
  model.count(filter, function findCb (err, count) {
