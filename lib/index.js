      ...
      if (schema) {
        ...
        function solrCreate() {
          ...
          self.solr.sendToSolr({
            path: '/admin/' + (options.enableDse ? 'cores' : 'collections') + '?action=CREATE&name=' + core + shardArgs
          }, function (err, result) {
            if (!err) {
              if (self.solrLogger)
                self.solrLogger.log('CREATE ' + core + ' Succeeded');
              self.coreInited[table] = true;
              self.coreIniting[table] = false;
              return callback(err);
            }
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (conds.$solr || conds.solr_query) {
      ...
      function solrFind() {
        ...
        var count = (self.options.solr.shardArgs && self.options.solr.shardArgs.numShards) || 1;
        q.shards(count > 1);
        if (typeof solrQuery === 'object') {
