  ...
  this.getModel(collection, function (err, model) {
      ...
      function solrFind() {
        ...
        if (typeof solrQuery === 'object') {
          Object.keys(solrQuery).forEach(function (k) {
            if (/^hl$|^hl.fl$|^hl.simple.pre$|^hl.simple.post$|^raw$|^multicore$|^sort$|^reindex$|^solronly$|^q$/.test(k))
              return;
    ...
    function finishFind(conds, isSolr, highlighting) {
      ...
      function execute(err) {
        if (err) return cb(err, null);
        if (opts.count) {
          return model.count(filter.where, filter.limit, filter.offset, findCb);
        } else {
          return model.find(filter, findCb);
        }
