  ...
  if (!model) {
    ...
    function getTheModel() {
      ...
      function done(err, fields) {
        ...
        model = self.models[collection] = cassandra.define(collection, { fields: fields, primaryKeys: keys }, null, true);
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (conds.$solr || conds.solr_query) {
      ...
      if (solrQuery && (solrQuery.reindex || (self.options.solr.reindex && !self.options.solr.reindex[collection]))) {
        ...
        var chunkSize = 500;
        var lastId = null;
        var queryState = model.schema.info.primaryKeys.length + 1;
        function indexDocs(err) {
          ...
          if (lastId) {
            ...
            model.schema.info.primaryKeys.forEach(function (e, i) {
              ...
              if ((queryState - 1) < i)
                return;
              var l = queryState === (i + 1);
              if (!f.isColumnKey) {
                var v = model.schema.toDatabase(lastId[i], e, model.schema.info.fields);
                if (l) {
                  q = where["token(" + e + ")"] = {};
                  q[f.reversed ? 'lt' : 'gt'] = 'token('+ v +')';
                } else {
                  where["token(" + e + ")"] = 'token('+ v +')';
                }
              } else {
                if (l) {
                  q = where[e] = {};
        ...
        function insertRows(rows) {
          ...
          if (rows.length === 0) {
            ...
            queryState = model.schema.info.primaryKeys.length + 1;
          if (self.solrLogger) {
        ...
        self.solr.search(q, function (err, docs) {
          ...
          if (docs) {
            ...
            if (solrQuery.multicore || solrQuery.solronly) {
              ...
              finishFind({all:allConds,ids:allIds}, true, docs.highlighting);
            }
          } else {
            return cb(null, []);
          }
