  ...
  if (this.solr) {
    cassandra.schemaSync = function (model, callback) {
      ...
      if (schema) {
        ...
        if (this.solrLogger)
          this.solrLogger.log('CREATING/RELOADING Solr core ' + core);
        async.forEach(items, function (options, next) {
          self.solr.sendToSolr(options, function (err) {
            if (err) {
              if (this.solrLogger)
                this.solrLogger.log('SENDING ' + options.info + ' => ' + err.message);
            }
            next();
          });
        }, function (err) {
          self.solr.sendToSolr({
            ...
            if (this.solrLogger)
              this.solrLogger.log('Create failed, trying reload ' + core + ' => ' + err.message);
            self.solr.sendToSolr({
              path: '/admin/cores?action=RELOAD&name=' + core
            }, function (err) {
              if (err) {
                if (this.solrLogger)
                  this.solrLogger.log('Reload failed ' + core + ' => ' + err.message);
              }
              callback(err);
            });
          });
  ...
  this.getModel(collection, function (err, model) {
    ...
    function finishFind(conds, isSolr) {
      var filter = {
        ...
        limit: opts.limit,
        offset: opts.offset,
        allowFiltering: !model.connection.options.enableDse
      };
      var needMigrate = false;
      if (!isSolr) {
        var indexes = model.schema.getIndexes();
