...
Cassandra.prototype.connect = function connect (options, callback) {
  ...
  if (this.solr) {
    this.schemaSync = cassandra.schemaSync = function (model, callback) {
      ...
      if (self.coreIniting[table]) {
        var that = this;
        setTimeout(function () {
          self.schemaSync.call(that, model, callback);
        }, 250);
        return;
      }
      self.coreIniting[table] = true;
      ...
      if (schema) {
            ...
            if (item) {
              ...
              return zk.get(item.path, function (err, value, zstat) {
                ...
                zk.set(item.path, item.content, zstat.version, function (err, zstat) {
                  if (self.solrLogger)
                    self.solrLogger.log('Created ' + item.path + ' ' + zstat.version);
        ...
        function solrCreate() {
          if (self.solrLogger)
            self.solrLogger.log('Sending CREATE');
          self.solr.sendToSolr({
            path: '/admin/cores?action=CREATE&name=' + core
          }, function (err, result) {
            if (!err) {
              if (self.solrLogger)
                self.solrLogger.log('CREATE Succeeded');
              self.coreInited[table] = true;
              self.coreIniting[table] = false;
              return callback(err);
            }
            if (self.solrLogger) {
              if (result)
                ...
                result = '';
              createError = err.message + result;
            }
            if (self.solrLogger)
              self.solrLogger.log('Sending RELOAD');
            self.solr.sendToSolr({
              path: '/admin/' + (options.enableDse ? 'cores' : 'collections') + '?action=RELOAD&name=' + core
            }, function (err, result) {
              if (err) {
                if (self.solrLogger) {
                  if (result)
      ...
      if (doc) {
        ...
        if (!o._extract) {
          ...
          if (self.logger) {
            ...
            self.solrLogger.log('ADD EXTRACT', oo);
          }
          delete o._extract;
          o = p;
        }
        self.solr.add(o, options, function(err, obj) {
          if (err) {
            console.log(obj);
            if (self.solrLogger) {
              self.solrLogger.log('ADD FAILED', err.json || err.message)
            }
          }
        ...
        if (self.solrLogger) {
          self.solrLogger.log('DELETE', {id:id});
        }
        var data = {delete:{id:id}};
        var option = {core:self.keyspace + '.' + collection};
...
Cassandra.prototype.getModel = function getModel (collection, callback) {
  ...
  var self = this;
  if (!model) {
    if (this.gettingModel[collection]) {
      return setTimeout(function () {
        self.getModel(collection, callback);
      }, 250);
    }
    this.gettingModel[collection] = true;
    if (this.getModelFields) {
      this.getModelFields(collection, done);
    } else {
      done(null, {});
    }
    function done(err, fields) {
      if (err) {
        self.gettingModel[collection] = false;
        return callback(err);
      }
      model = self.models[collection] = cassandra.define(collection, { fields: fields, primaryKeys: ['id'] }, null, true);
        ...
        function update(err) {
          ...
          if (options.remove) {
            ...
            model.update(doc, conds, function (err, info) {
              var o = self.flatten(oldDoc.data || {});
              doc._id = doc.id || id;
              delete doc.id;
...
Cassandra.prototype.rawUpdate = function update (collection, conds, sort, doc, options, callback) {
  ...
  this.getModel(collection, function (err, model) {
    ...
    conds = self.convertQuery(conds);
    model.update(doc, conds, function (err, info) {
      self.indexDoc(collection, doc, conds.id, 'rawUpdate', model);
      doc._id = doc.id;
      delete doc.id;
      callback(err, doc);
    }, options);
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (toInsert._id) {
      ...
      return model.update(json, {where:{id:toInsert._id}}, function (err, info) {
        self.indexDoc(collection, json, id, 'findAndModify', model);
    ...
    model.insert(json, opts, function insertCb (err, doc) {
      self.indexDoc(collection, json, json.id, 'insert', model);
      if (err) return cb(err);
      cb(null, {_id: doc.id});
    });
  });
...
Cassandra.prototype.find = function find (collection, conds, opts, cb) {
  ...
  this.getModel(collection, function (err, model) {
    ...
    if (conds.$solr || (self.options.enableDse && conds.solr_query)) {
      if (!self.solr)
        return cb(new Error('Solr not enabled for searching'));
      if (conds.$solr && (conds.$solr.$reindex || (self.options.solr.reindex && !self.options.solr.reindex[collection]))) {
        self.options.solr.reindex[collection] = true;
        var first = true;
        var chunkSize = 100000;
        function indexDocs(err, offset) {
          // Offset is not really supported in CQL 3+
          if (offset) {
            return insertRows([], offset);
          }
          model.find({where:{}, limit:chunkSize, offset:offset}, function findCb (err, rows) {
            if (err) {
              console.log(err);
              return cb(err);
            }
            if (first) {
              ...
              return self.solr.update(data, options, function (err, data) {
                ...
                insertRows(rows, offset);
              });
            }
            insertRows(rows, offset);
          });
        }
        function insertRows(rows, offset) {
          // Offset is not really supported in CQL 3+
          ...
          if (self.solrLogger) {
            self.solrLogger.log('INDEX ' + collection + ' offset ' + offset + ' count ' + rows.length);
          }
          function addDoc(err) {
            if (err) {
              console.log(err);
            ...
            if (!row) {
              return indexDocs(null, offset + chunkSize);
            } else {
              var doc = self.unflatten(row.data) || {};
      ...
      function solrFind() {
        var q;
        if (self.options.enableDse) {
          q = conds.$solr || conds.solr_query;
          delete conds.solr_query;
          conds.$solr = q;
        } else {
          q = conds.$solr;
        }
        solrProperty = q;
        var hl = false;
        if (typeof conds.$solr === 'object') {
          q = conds.$solr.q;
          hl = conds.$solr.hl;
        }
        //if (conds.$solr.$reindex)
        //  delete conds.$solr.$reindex;
        var q = self.solr.createQuery().q(q).start(opts.offset || 0).rows(opts.limit || 50);
        var fl = conds.$solr.fl;
        if (Array.isArray(conds.$solr.multicore)) {
          q.multicore(conds.$solr.multicore);
          if (fl && fl !== '*') {
            fl += ",_core";
          }
        }
        if (hl) {
          q.set('hl=true');
          q.set('hl.fl=' + (fl || '*'));
          q.set('hl.simple.pre=' + (conds.$solr['hl.simple.pre'] || '<strong>'));
          q.set('hl.simple.post=' + (conds.$solr['hl.simple.post'] || '</strong>'));
        }
        if (typeof conds.$solr === 'object') {
          Object.keys(conds.$solr).forEach(function (k) {
            if (/^hl$|^hl.fl$|^hl.simple.pre$|^hl.simple.post$|^multicore$|^sort$|^\$reindex$/.test(k))
              return;
            q.set(k + '=' + encodeURIComponent(conds.$solr[k]));
          });
        }
        if (opts.sort) {
          var ss = [];
          ...
          q.set('sort=' + ss.join(','));
        } else if (conds.$solr.sort) {
          q.set('sort=' + encodeURIComponent(conds.$solr.sort));
        }
        ...
        self.solr.search(q, function (err, docs) {
          if (err) {
            if (self.solrLogger) {
              ...
              self.solrLogger.log('SEARCH FAILED', info);
            }
            return cb(null, []);
          }
          if (docs) {
            ...
            if (newConds.id.$all.length === 0) {
              return cb(null, []);
            }
            if (conds.$solr.multicore) {
              var items = [];
              ...
              finishFind(newConds, true, docs.highlighting);
            }
          } else {
            return cb(null, []);
          }
    ...
    function finishFind(conds, isSolr, highlighting) {
      ...
      function execute(err) {
        ...
        model.find(filter, function findCb (err, rows) {
          ...
          if (isSolr) {
            filter.where.id.forEach(function (id) {
              var doc = unsort[id];
              if (doc)
                docs.push(doc);
            });
