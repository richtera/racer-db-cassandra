...
//                  self.solrLogger.log('Updating ' + item.path);
        ...
        function solrCreate() {
          ...
          self.solr.sendToSolr({
            path: '/admin/' + (options.enableDse ? 'cores' : 'collections') + '?action=RELOAD&wt=json&name=' + core + shardArgs
          }, function (err, result) {
            ...
            self.solr.sendToSolr({
              path: '/admin/' + (options.enableDse ? 'cores' : 'collections') + '?wt=json&action=CREATE&name=' + core + shardArgs
            }, function (err, result) {
              if (err) {
                if (self.solrLogger) {
                  if (result)
