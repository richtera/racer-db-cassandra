...
Cassandra.prototype.connect = function connect (options, callback) {
  ...
  var self = this;
  if (this.solr) {
    cassandra.schemaSync = function (model, callback) {
      ...
      if (schema) {
        ...
        var core = model.connection.keyspace.name + '.' + model.schema.info.tableName;
        self.solr.sendToSolr({ path: '/resource/' + core + '/schema.xml', method: 'GET'}, function (err, result) {
          ...
          var items = [
            {
              path: '/resource/' + core + '/synonyms.txt',
              contentType: 'text/plain',
              content: synonyms,
              info: 'synonyms.txt'
            },
            {
              path: '/resource/' + core + '/stopwords.txt',
              contentType: 'text/plain',
              content: stopwords,
              info: 'stopwords.txt'
            },
            {
              path: '/resource/' + core + '/solrconfig.xml',
              contentType: 'text/xml',
              content: config,
              info: 'solrconfig.xml'
            },
            {
              path: '/resource/' + core + '/schema.xml',
              contentType: 'text/xml',
              content: schema,
              info: 'Schema.xml'
            }
          ...
          async.forEach(items, function (options, next) {
            self.solr.sendToSolr(options, function (err, result) {
              if (result)
                result = '\n' + result;
              else
                result = '';
              if (err) {
                if (self.solrLogger)
                  self.solrLogger.log('SENDING failed ' + options.info + ' => ' + err.message + result);
              }
              next();
            });
          }, function (err) {
            ...
            function solrCreate() {
              self.solr.sendToSolr({
                ...
                self.solr.sendToSolr({
                  path: '/admin/cores?action=RELOAD&name=' + core
                }, function (err, result) {
                  if (err) {
                    if (self.solrLogger) {
                      if (result)
...
//    return;
  ...
  if (this.solr) {
    ...
    var self = this;
    if (doc) {
      var o = this.formatIndexDoc(collection, doc, id, model);
      if (this.logger) {
      ...
      if (this.options.enableDse) {
        ...
        options.core = this.keyspace + '.' + collection;
        options.update = 'update';
      }
      this.solr.add(o, options, function(err, obj) {
        if (err) console.log(err);
        if (!self.options.enableDse)
          scheduleSolrCommit();
      });
    } else if (!this.options.enableDse) {
      if (this.logger) {
        this.solrLogger.log('DELETE', {id:id});
      }
