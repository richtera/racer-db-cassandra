...
exports.solrSchema = function (schema) {
  ...
  for (var i in schema.info.fields) {
    ...
    if (!field.solrDest)
      continue;
    if (!destFields[field.solrDest]) {
      output.push(
    '    <field name="' + field.solrDest + '" type="text" indexed="true" stored="false" multiValued="true"/>'
      );
      destFields[field.solrDest] = true;
    }
  }
  for (var i in schema.info.fields) {
    if (/^_.*/.test(i) || /\./.test(i))
  ...
  for (var i in schema.info.fields) {
    ...
    if (!field.solrExclude) {
      output.push(
    '  <copyField source="' + name + '" dest="' + (field.solrDest || '_body') + '"/>'
      );
    }
  }
  output.push(
