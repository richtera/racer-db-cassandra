...
exports.stopwords = function (schema) {
  return [
...
exports.synonyms = function (schema) {
  return [
    '# The ASF licenses this file to You under the Apache License, Version 2.0',
    '# (the "License"); you may not use this file except in compliance with',
    '# the License.  You may obtain a copy of the License at',
...
exports.solrConfig = function (schema) {
  return [
  ...
  + '                    class="com.datastax.bdp.cassandra.index.solr.DSENRTCachingDirectoryFactory"/>') :
    ...
    '  </requestHandler>',
    '',
    '  <!-- SOLR Cloud replication handler -->',
...
exports.solrSchema = function (schema) {
  var output = [
    '<?xml version="1.0" encoding="UTF-8" ?>',
    '<!-- Dynamically generated schema -->',
    '<!-- Version 1.1 -->',
    '<schema name="' + schema.info.tableName + '" version="1.1">',
    '  <types>',
    ...
    '        <filter class="solr.EdgeNGramFilterFactory" side="front" minGramSize="1" maxGramSize="4"/>',
    '      </analyzer>',
    '      <analyzer type="query">',
    '        <tokenizer class="solr.WhitespaceTokenizerFactory"/>',
    '        <filter class="solr.LowerCaseFilterFactory"/>',
    ...
    '  </types>',
    '  <fields>',
    '    <field name="_core" type="string" indexed="true" stored="true" multiValued="false"/>',
    '    <field name="_body_partial" type="text_partial" indexed="true" stored="false" multiValued="true"/>',
    '    <field name="_body" type="text" indexed="true" stored="true" multiValued="true"/>',
    '    <field name="_version_" type="string" indexed="true" stored="true" multiValued="false"/>'
  ];
  
  // Set to true if you want to exclude all fields with secondary indexes from SOLR.
  var excludeIndexed = false;

  var destFields = {};
  for (var i in schema.info.fields) {
    if (/^_.*/.test(i) || /\./.test(i))
      continue;
    var field = schema.info.fields[i];
    switch (field.type.name) {
    case 'blob':
      field.solr_exclude = true;
      break;
    case 'uuid':
      field._defaultType = 'uuid';
      break;
    case 'timeuuid':
      field._defaultType = 'timeuuid';
      break;
    case 'timestamp':
      field._defaultType = 'date';
      break;                
    case 'boolean':
      field._defaultType = 'boolean';
      break;
    case 'date':
      field._defaultType = 'date';
      break;
    case 'float':
      field._defaultType = 'float';
      break;
    case 'double':
      field._defaultType = 'double';
      break;
    case 'int':
      field._defaultType = 'int';
      break;
    case 'varchar':
    case 'text':
      field._defaultType = 'text';
      break;
    }
    if (!field.solr_copyto)
      continue;
    if (!destFields[field.solr_copyto]) {
      output.push(
    '    <field name="' + field.solr_copyto + '" type="text" indexed="true" stored="true" multiValued="true"/>'
      );
      destFields[field.solr_copyto] = true;
    }
  }
  var hasId = false;
  for (var i in schema.info.fields) {
    if (/^_.*/.test(i) || /\./.test(i))
      continue;
    var field = schema.info.fields[i];
    var type = field.solr_type || field._defaultType || "string";
    field.solrIndex = true;
    if (i === 'id') {
      output.push(
    ...
    var name = i;
    if (!(field.index && excludeIndexed)) {
      output.push(
    '    <field name="' + name + '" type="' + type + '" indexed="true"  stored="true"/>'
      );
    }
  }
  if (!hasId) {
  ...
  for (var i in schema.info.fields) {
    if (/^_.*/.test(i) || /\./.test(i) || i === 'id')
      continue;
    var name = i;
    var field = schema.info.fields[i];
    name = name.replace(/\./g, '_');
    if (!field.solr_exclude && field._defaultType === 'uuid')
      field.solr_exclude = true;
    if (!field.solr_exclude && !(field.index && excludeIndexed)) {
      output.push(
    '  <copyField source="' + name + '" dest="' + (field.solr_copyto || '_body') + '"/>'
      );
      if (field.solr_copyto !== '_body_partial')
        output.push(
    '  <copyField source="' + name + '" dest="_body_partial"/>'
        );
    }
  }
  output.push(
